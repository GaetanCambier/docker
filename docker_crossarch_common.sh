...
read -r -a __crossarch_archs <<< "${CROSSARCH_ARCHS:="amd64 armhf aarch64"}"
#__crossarch_alpine_branch=(${CROSSARCH_ALPINE_BRANCH:="v3.7 v3.9 v3.10"})
read -r -a __crossarch_alpine_branch <<< "${CROSSARCH_ALPINE_BRANCH:="v3.7 v3.9 v3.12 edge"}"
...
crossarch_common_deploy_latest () {
  local docker_username="${1}"
  local docker_password="${2}"
  local build_name="${3}"
  local build_version="${4}"
...
crossarch_common_entry () {
  ...
  crossarch_common_deploy_latest "${docker_username}" "${docker_password}" "${build_name}" "${build_version}"
}
